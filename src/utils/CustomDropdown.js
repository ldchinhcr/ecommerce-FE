...
export default function CustomDropdown(props) {
  ...
  const handleClose = param => {
    ...
    handleLinkRouter(param)
  };

  const handleLinkRouter = (param) => {
    const string = param.split(' ').join('')
    const link = string.toLowerCase();
    const allowedRouter = ["cart"]
    if (allowedRouter.includes(link)) {
        console.log('waiting for the cart')
    } else if (link === "logout") {
        logOut()
    } else {
        history.push(`/${link}`)
